...
with st.expander("🔰 初めての方はこちら（目的・使い方・NotebookLM活用）", expanded=False):
    ...
    with tab0:
        st.markdown("""
        ### 何のためのアプリなの？
        このアプリは、**「PDFなどの資料を、編集可能なパワーポイントに変換する」**ための道具です。
        
        最近流行りのAIツール（**NotebookLM**など）で作った資料は、内容が良くても「PDF」でしか保存できないことが多く、**「ページの順番を変えたい」「会社指定の表紙をつけたい」**といった時に困ってしまいます。
    ...
    with tab1:
        ...
        * 📁 複数のPDFを一度に入れてもOKです（1つのパワポにまとまります）。
        
        **2. プレビューを確認**
        画面中央に「仕上がりプレビュー」が出ます。
        * **赤枠の部分**が白く塗りつぶされて消えます（ページ番号などを消す用）。
        * 左メニューの「✂️ ロゴ消し」で調整してください。
        
        **3. 変換スタート**
        ボタンを押すと変換が始まります。終わるとダウンロードボタンが出ます。
        """)
        
    with tab2:
        st.markdown("""
        ### プロっぽく仕上げるテクニック
        左側のメニュー（サイドバー）で調整できます。
        
        **✨ 文字をくっきりさせる**
        * **「✨ 画質補正」**の**「コントラスト」**を右に動かすと、薄い文字が濃くなります。
        
        **🏢 会社名を入れる**
        * **「フッター文字」**に入力すると、全ページの左下に自動で入ります。
        * **「透かし文字」**に「社外秘」と入れると、薄くスタンプされます。
    ...
    with tab3:
        st.markdown("""
        ### パワポにした後、文字を直したい！
        このアプリは、レイアウト崩れを100%防ぐために「ページ全体を画像」として貼り付けます。
        ...
        **💡 ヒント：元の文章データはどこ？**
        ...
        AIが読み取った文章がそこに全部入っていますので、コピペして使えます。
        """)
